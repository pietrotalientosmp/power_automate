"""
Endpoint Flask per estrazione articoli RSS — deploy su Railway.
...
def normalize_articles(raw_items: list) -> list:
    """
    Gestisce due formati in ingresso:
    1. Formato flat Power Automate: chiavi tipo "0.title", "1.summary"
    2. Formato strutturato RSS connector: array di oggetti con "title", "summary" ecc.
    """
    if not raw_items:
        return []
    first = raw_items[0]
    if isinstance(first, dict) and any("." in k for k in first):
...
def _normalize_single(item: dict) -> dict:
    ...
    url = str(
        ...
        or (links[0] if isinstance(links, list) and links else None)
        or item.get("id", "")
    ).strip()
...
def _clean_summary(text: str) -> str:
    ...
    text = re.sub(r"<[^>]+>", "", text)   # rimuove tag HTML (es. <img ...>)
